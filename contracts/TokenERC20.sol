...
contract TokenERC20 {
    ...
    uint256 multiplierNumerator = 100000; // Super magic numbers for now
    uint256 multiplierDenominator = 100000; // Super magic numbers for now
    ...
    function stakeLief(uint256 stemId, uint256 stakedLiefs) stemExists(stemId) public returns (bool success) {
        transfer(this, stakedLiefs);
    ...
    function getStakedLiefsOfStem(uint256 stemId) public returns (uint256 stakedLiefs) {
        return stems[stemId].totalStakedLief;
    }
    
    function getDistributedEquityOfStem(uint256 stemId) public returns (uint256 distributedEquity) {
    ...
    function _determineEquityToGive(uint256 stemId, uint256 stakedLiefs) internal returns (uint256 equityToGive) {
        ...
        stemId = stemId * 1;
        return stakedLiefs * (multiplierNumerator / multiplierDenominator);
    }
    ...
    function _giveLiefToStem(uint256 stemId, uint256 liefAmount) internal returns (bool success) {
        ...
        balanceOf[msg.sender] -= liefAmount; // TODO: Need to check if there's enough balance
        return true;
    }
